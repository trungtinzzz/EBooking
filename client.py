...
def recvimage(hotelname, dicthotel):
    ...
    for i in range(len(dicthotel)):
        f = open('client/'+dicthotel[hotelname][i]['no']+'.jpg', 'wb')
        data = sock.recv(100000000)
        f.write(data)
        imagelist.append(ImageTk.PhotoImage(Image.open('client/'+dicthotel[hotelname][i]['no']+'.jpg')))
...
def menu(username):
    while True:
        ...
        if m_choices == '1':
            ...
            # luan
            strin = sock.recv(1024)
            hotel_dict = json.loads(strin)
            recvimage(ans, hotel_dict)
